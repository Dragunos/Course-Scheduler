...
public class ExcelCourseImporter
{
	...
	public List<ImportedLopRow> importFrom(File file) throws IOException, InvalidFormatException
	{
		...
		try (FileInputStream fis = new FileInputStream(file); Workbook wb = WorkbookFactory.create(fis)) {
			...
			for(int i = ROWS_TO_SKIP; i <= last; i++) {
				...
				ImportedLopRow imported = new ImportedLopRow();
				...
				if(thu != null && !tietList.isEmpty()) {
					List<ImportedLopRow.Buoi> buois = convertTietListToBuoi(thu, tietList);
	...
	private static List<ImportedLopRow.Buoi> convertTietListToBuoi(int thu, List<Integer> tietList)
	{
		List<ImportedLopRow.Buoi> result = new ArrayList<>();
		...
		for(int i = 1; i < sorted.size(); i++) {
			...
			if(cur == prev + 1) {
				...
				result.add(new ImportedLopRow.Buoi(thu, start, prev));
				start = cur;
				prev = cur;
			}
		}
