...
public class ImportDanhSachLopService implements ImportDanhSachLopUseCase
{
	...
	public DanhSachLopDto importDanhSach(ImportDanhSachLopRequestDto request)
	{
		...
		List<ImportedLopHocPhanRow> rows;

		try {
			rows = importer.importFrom(f);
		}
		catch(Exception ex) {
			throw new DataAccessException("Failed to read file", ex);
		}

		Map<String, ImportedLopAggregate> map = new LinkedHashMap<>();
		for(ImportedLopHocPhanRow r : rows) {
		...
		try {
			...
			throw new DataAccessException("Import failed", ex);
		}
