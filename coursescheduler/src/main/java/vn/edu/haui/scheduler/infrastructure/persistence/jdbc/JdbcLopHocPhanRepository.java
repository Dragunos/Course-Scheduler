...
public class JdbcLopHocPhanRepository implements LopHocPhanRepositoryPort
{
	...
	{
		String insert = "INSERT INTO lop_hoc_phan (ma_lop, hoc_phan_id, giang_vien_id, hinh_thuc_day, dia_diem) " +
				"VALUES (?,?,?,?,?)";
		...
		try (Connection conn = DataSourceProvider.getDataSource().getConnection();
			...
			ps.setLong(2, lop.getHocPhanId());

			if(lop.getGiangVienId() != null) ps.setLong(3, lop.getGiangVienId());
